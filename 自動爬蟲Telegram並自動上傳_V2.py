...
async def main():
    ...
    # ==========================================
    # 區塊 B：透過 Yahoo Finance 抓取股市報價 (💡 本次新增)
    # ==========================================
    print("\n📈 開始透過 Yahoo Finance 抓取股市歷史報價...")
    
    # 在這裡設定你想抓的標的。左邊是你的 CSV 檔名，右邊是真實的股票代號
    # (註：美股沒有純 DRAM ETF，通常用半導體 ETF 如 SMH 替代；若想抓台股半導體 ETF，可以寫 '00891.TW')
    stock_targets = {
        'NVDA': 'NVDA',
        'DRAM_ETF': 'DRAM'  
    }
        ...
        try:
            ...
            if not hist.empty:
                ...
                if len(hist) >= 2:
                    ...
                    day_val = latest_price - prev_price
                ...
                print(f"  ✔️ 成功獲取 {item_name} 最新收盤價: {latest_price}")
    ...
    if all_parsed_data:
        ...
        if has_updated_files:
            ...
            try:
                ...
                subprocess.run(["git", "commit", "-m", "自動更新報價 (含NVDA與ETF)"], check=True)
