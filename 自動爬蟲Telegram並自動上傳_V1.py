import os
import re
import subprocess  # 💡 新增：用來讓 Python 執行終端機指令 (Git)
import pandas as pd
...
async def main():
    ...
    # --- 進入智慧存檔與比對階段 ---
    has_updated_files = False  # 💡 新增：用來追蹤這次有沒有真的更新檔案
    ...
    if all_parsed_data:
        ...
        for item_name, group_data in df_scraped.groupby('Item'):
            ...
            if os.path.exists(csv_path):
                ...
                if affected_dates:
                    has_updated_files = True  # 💡 標記為：有檔案更新
                ...
                has_updated_files = True  # 💡 標記為：有檔案更新 (建立新檔)
